...
public class TestMovieAndMovieDataReader {
	...
	public void runTests() {
		...
		if (this.testMovieAvgVote()) {
			System.out.println("Test movie average vote: PASSED");
		} else {
			System.out.println("Test movie average vote: FAILED");
		}
	...
	List<MovieInterface> movieList;
		...
		boolean equalDirector = true;
		// check if first movie has one of the above directors
		equalDirector = equalDirector && (director1.equals(movieList.get(0).getDirector()) || director2.equals(movieList.get(0).getDirector()) || director3.equals(movieList.get(0).getDirector()));
		// check if second movie has one of the above directors
		equalDirector = equalDirector && (director1.equals(movieList.get(1).getDirector()) || director2.equals(movieList.get(1).getDirector()) || director3.equals(movieList.get(1).getDirector()));
		//check if third movie has one of the above directors
		equalDirector = equalDirector && (director1.equals(movieList.get(2).getDirector()) || director2.equals(movieList.get(2).getDirector()) || director3.equals(movieList.get(2).getDirector()));
		
		// return true if directors appear in one of the movies
		return equalDirector;

	}

	/**
	 * This test reads in 3 movies and tests whether the list of movies
	 * contains all 3 average votes of the source data in any order. It fails
	 * if the list returned is missing one or more titles or if an
	 * exception occurs while reading in the data.
	 * @return true if the test passed, false if it failed
	 */
	public boolean testMovieAvgVote() {
		List<MovieInterface> movieList;
		try {
			movieList = readerToTest.readDataSet(new StringReader(
					"title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
		...
		Float avgVote1 = new Float(2.9);
		Float avgVote2 = new Float(3.5);
		Float avgVote3 = new Float(5.4);
		boolean equalVote = true;

		// checks if first movie has one of the above average votes
		equalVote = equalVote && (avgVote1.equals(movieList.get(0).getAvgVote()) || avgVote2.equals(movieList.get(0).getAvgVote()) || avgVote3.equals(movieList.get(0).getAvgVote()));
		// checks if second movie has one of the above average votes
		equalVote = equalVote && (avgVote1.equals(movieList.get(1).getAvgVote()) || avgVote2.equals(movieList.get(1).getAvgVote()) || avgVote3.equals(movieList.get(1).getAvgVote()));
		// checks if third movie has one of the above average votes
		equalVote = equalVote && (avgVote1.equals(movieList.get(2).getAvgVote()) || avgVote2.equals(movieList.get(1).getAvgVote()) || avgVote3.equals(movieList.get(2).getAvgVote()));

		// returns true if the avg votes  appear in one of the movies
		return equalVote;
	}
}
